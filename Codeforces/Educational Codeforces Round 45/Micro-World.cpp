...
int main ()
{
  ...
  for (int i = 0; i < final.size(); i++) {
    for (int j = i+1; j < final.size()-1; j++) {
      if (final[i]+K >= final[j]) {
        // cout << final[i] << " dimakan oleh " << final[j] << endl;
        break;
      }
      if (final[i]+K < final[j]) {
        counter+=myset[final[i]];
        break;
      }
    }
  }
  counter +=myset[final[final.size()-1]];  
  cout << counter << endl;
