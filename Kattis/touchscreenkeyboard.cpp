...
int main ()
{
  ...
  Tloop {
    string master;
    cin >> master;
    vector<mystruct> wordlist;
    Nloop {
      cin >> x;
      mystruct newword;
      int total = 0;
      for (int k = 0; k <x.length(); k++) {
        total += abs(row[dummy+master[k]] - row[dummy+x[k]]);
        total += abs(col[dummy+master[k]] - col[dummy+x[k]]);
      }
      newword.dist = total;
      newword.word = x;
      wordlist.push_back(newword);
    }
    sort(wordlist.begin(), wordlist.end(), customcompare);
    for (auto y : wordlist) {
      cout << y.word << " " << y.dist << endl;
    }
  }
