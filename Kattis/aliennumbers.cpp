...
int main ()
{
    ...
    Tloop {
        string val, lang1, lang2;
        cin >> val >> lang1 >> lang2;

        ll bil = 0;

        ll basis = lang1.size();
        ll basis2 = lang2.size();

        reverse(val.begin(), val.end());

        for (ll i = 0; i < val.size(); i++) {
            auto elem = val[i];
            for (ll j = 0; j < basis; j++) {
                if (elem == lang1[j]) {
                    bil += (pow(basis, i) * j); 
                    break;
                }
        ...
        for (ll i = 0; i < 1000000000; i++) {
            ...
            ll dummy = (bil % ll(pow(basis2, i+1))) / pow(basis2, i);
            ans += lang2[dummy];
            // cout << dummy << " " << lang2[dummy] << endl;
            bil -= bil % ll(pow(basis2, i+1));
        }
