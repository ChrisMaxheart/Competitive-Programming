...
int main ()
{
    ...
    ll flow = mf.calcMaxFlow(s, t);
    vector<pair<int, pii>> ans;
    for (auto zz : mf.graph) {
        for (auto x : zz) {
            if (x.cap > 0 && x.flow > 0) {
                ans.pb(mp(x.from, mp(x.to, x.flow)));
                // cout << x.from << " " << x.to << " " << x.flow << endl;
            }
        }
    }

    cout << n << " " << flow << " " << ans.size() << endl;
    for (auto x: ans) {
