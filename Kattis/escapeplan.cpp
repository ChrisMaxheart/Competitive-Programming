...
int main ()
{
    ...
    while(cin >> N, N) {      
        ...
        HopcroftKarp mcbm1(N, M);
        HopcroftKarp mcbm2(N, M);
        HopcroftKarp mcbm3(N, M);

        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                auto robot = robotlst[i];
                auto lubang = lubanglst[j];

                double dx = robot.fi - lubang.fi;
                double dy = robot.se - lubang.se;

                double jarak = sqrt(dx*dx + dy*dy);
                // cout << i << " " << j << " " << dx << " " << dy <<  " " << jarak << endl;
                if (jarak <= 200.0) {
                    mcbm1.addEdge(i, j);
                }
        ...
        cout << "In 5 seconds " << mcbm3.maxMatching() << " robot(s) can escape" << endl;
        cout << "In 10 seconds " << mcbm2.maxMatching() << " robot(s) can escape" << endl;
        cout << "In 20 seconds " << mcbm1.maxMatching() << " robot(s) can escape" << endl;
        cout << endl;
    }
