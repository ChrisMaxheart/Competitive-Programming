...
//        cout << AL.size() << endl;
        for (auto x : AL) {
            ...
            auto curr = x.fi;
            if (victim.find(curr) != victim.end()) {
                continue;
            }
            ...
            while(bfs.size() > 0) {
                ...
                auto evalu = bfs.front();
                bfs.pop();
                in_queue[evalu.fi] = 0;
                if (visited[evalu.fi] == INF) {
                    visited[evalu.fi] = evalu.se;
                    if (evalu.se > 7) {
//                        cout << curr << " to " << evalu.fi << endl;
                        if (!sudah) {
                            counter++;
                            sudah = true;
                        }
                        if (victim.find(evalu.fi) == victim.end()) {
                            victim.insert(evalu.fi);
                            counter++;
                        }
                    }
                    for (auto y : AL[evalu.fi]) {
                        if (visited[y] == INF && !in_queue[y]) {
                            bfs.push(mp(y, evalu.se+1));
                            in_queue[y] = 1;
                        }
                    }
                }
