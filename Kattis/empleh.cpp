...
int main ()
{
  ...
  if (x.size() > 7) {
    ...
    while(getline(iss, dummy, ',')) {
      ...
      char row, col;
      row = dummy[dummy.size()-1];
      col = dummy[dummy.size()-2];
      int rowno = row-'0';
      int colno = col - 'a' +1;
      chessboard[rowno][colno] = piece;
    }
  }
  getline(cin, x);
  if (x.size() > 7) {
    istringstream dum2(x);
    dum2 >> x;
    dum2 >> x;

    istringstream isy(x);

    while(getline(isy, dummy, ',')) {
      char piece;
      if (dummy.size() == 3) {
        piece = dummy[0]-'A'+'a';
      } else {
