...
int main () {
        ...
        for (int j = 1; j < c + 1; j++) {
            ...
            for (int a = -1; a <= 1; a++) {
                for (int b = -1; b <= 0; b++) {
                    ...
                    if (pta[i + a][j + b] < 0) {
                        nxt = loc(i + a, j + b);
                        auto w = max(pta[i][j], pta[i + a][j + b]);
                        AL[nxt].pb(mp(curr, w));
                        AL[curr].pb(mp(nxt, w));
                    }
                }
    ...
    dijk.push(mp(pta[rs][cs], loc(rs, cs)));
    ...
    while (dijk.size() > 0) {
        ...
        if (dist[curr.se] == INF) {
            ...
            ttl += ll(curr.fi);
//            cout << curr.se/505 << "&&" << curr.se%505 <<" " << curr.fi << endl;
//            cout << curr.se << " " << curr.fi << endl;
            for (auto x : AL[curr.se]) {
                if (dist[x.fi] == INF) {
                    dijk.push(mp(max(x.se, curr.fi), x.fi));
                }
            }
        }
    }
