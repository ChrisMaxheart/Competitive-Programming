...
int recurs(vi emp, int idx) {
    if (idx == AL.size()) {
        bool can = true;
        for (int i = 0; i < emp.size(); i++) {
            for (int j = i + 1; j < emp.size(); j++) {
                if (AL[emp[i]].find(emp[j]) == AL[emp[i]].end()) {
                    can = false;
                }
            }
        }
        ...
        int gaambil = recurs(emp, idx+1);
        emp.pb(idx);
        int ambil = recurs(emp, idx+1);
        return max(gaambil, ambil);
    }
}
