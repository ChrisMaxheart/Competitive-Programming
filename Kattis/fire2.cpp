...
int main ()
{
    ...
    Tloop {
        ...
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                if ((pta[i][j] == '.' || pta[i][j] == '@') && (i == 0 || j == 0 || i == r-1 || j == c-1)) {
                    AL[loc(i,j)].pb(dloc);
                    // cout << i << " " << j << endl;
                }
                if (pta[i][j] == '@') {
        ...
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c-1; j++) {
                auto curr = loc(i, j);
                auto neigh = loc(i, j+1);
                if (pta[i][j] != '#' && pta[i][j+1] != '#') {
                    AL[curr].pb(neigh);
                    AL[neigh].pb(curr);
                }
            }
        ...
        for (int i = 0; i < r-1; i++) {
            for (int j = 0; j < c; j++) {
                auto curr = loc(i, j);
                auto neigh = loc(i+1, j);
                if (pta[i][j] != '#' && pta[i+1][j] != '#') {
                    AL[curr].pb(neigh);
                    AL[neigh].pb(curr);
                }
            }
        ...
        while(dijk.size() > 0) {
            auto curr = dijk.top();
            dijk.pop();
            if (distf[curr.se] == INF) {
                distf[curr.se] = curr.fi;
                for (auto x : AL[curr.se]) {
                    if (distf[x] == INF && x != dloc) {
                        // if (x == loc(1, 0)) {
                            // cout << curr.se << " " << curr.fi << endl;
                            // cout << "test" << endl;
                        // }
                        dijk.push(mp(1+curr.fi, x));
                    }
                }
            }
        }
