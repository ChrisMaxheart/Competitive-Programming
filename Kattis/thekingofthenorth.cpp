...
int duploc(int r, int c) {
    return loc(301, 301) + loc(r, c);
}
...
int main ()
{
    ...
    DinicMaxFlow<int> mf(duploc(r, c) + 3);
    int src = duploc(r, c) + 1;
    int snk = duploc(r, c) + 2;
    ...
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            ...
            mf.addEdge(duploc(i, j), loc(i, j), x);

            int curr = duploc(i, j);

            if (i != 0) {
                mf.addEdge(loc(i-1, j), curr, INFflow);
            } else {
                mf.addEdge(src, curr, INFflow);
            }
            ...
            if (j != 0) {
                mf.addEdge(loc(i, j-1), curr, INFflow);
            } else {
                mf.addEdge(src, curr, INFflow);
            }
    ...
    int x, y;
    cin >> x >> y;
    mf.addEdge(loc(x, y), snk, INFflow);
