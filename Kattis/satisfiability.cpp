...
vs needto;
vector<multimap<int, int>> parsed;
int N;

void parse() {
    for (auto req: needto) {
        multimap<int, int> emp;
        vs dummy = strsplit(req, ' ');
        for (auto dum: dummy) {
            if (dum == "v") {
                continue;
            }
            vs tst = strsplit(dum, 'X');
            int sz = int(tst.size());
            int bil = stoi(tst[sz-1]);
            bil --;
            if (tst[0] == "~") {
                emp.insert(mp(bil, -1));
            } else {
...
bool recurs(int idx, int mask) {
    if(idx == N) {
        ...
        int mask1 = mask | (1 << idx);
        bool can = recurs(idx+1, mask);
        if (can) {
            return true;
        } else {
            return recurs(idx+1, mask1);
        }
    }
}
