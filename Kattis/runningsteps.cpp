...
ull recurs(int lngth, int no2) {
    if (lngth == 1 || lngth == no2) {
        return 1;
    }

    if (memo.find(mp(lngth, no2)) == memo.end()) {
        memo[mp(lngth, no2)] = recurs(lngth-1, no2);
        if (no2 > 0) {
            memo[mp(lngth, no2)] += recurs(lngth-1, no2-1);
        }
    }
    return memo[mp(lngth, no2)];
}
...
int main ()
{
    ...
    Tloop {
        ...
        for (int dua = 0; dua <= steps/2; dua++) {
            int satu = steps - 2 * dua;
            if (dua < satu) {
                continue;
            }

            ttl += recurs(satu + dua, dua) * recurs(satu + dua, dua);
        }
