...
int main ()
{
    ...
    Tloop {
        ...
        DinicMaxFlow<int> mf((lim+1) * N + 2);

        int src = (lim+1) * N;
        int snk = (lim+1) * N + 1;

        strtloc--;
        mf.addEdge(src, strtloc * (lim+1), amt);

        int medamt;
        cin >> medamt;
        for (int i = 0; i < medamt; i++) {
            int x;
            cin >> x;
            x--;
            mf.addEdge(x * (lim+1) + lim, snk, INFflow);
        }
        int edge;

        for (int i = 0; i < N; i++) {
            for (int j = 0; j <= lim-1; j++) {
                mf.addEdge(i * (lim+1) + j, i * (lim+1) + j + 1, INFflow);
            }
        }
        ...
        for (int i = 0; i < edge; i++) {
            ...
            u--;
            v--;
            for (int j = 0; j <= lim-w; j++) {
                mf.addEdge(u * (lim+1) + j, v * (lim+1) + j + w, cap);
            }
        }
