...
int main ()
{
    ...
    Tloop {
        ...
        int rs, cs, rd, cd;
        cin >> rs >> cs >> rd >> cd;
        dijk.push_back(mp(0, loc(rs, cs)));

        while(dijk.size() > 0) {
            auto curr = dijk.front();
            dijk.pop_front();
            if (dist[curr.se] == INF) {
                ...
                for (auto x : AL[curr.se]) {
                    if (dist[x.fi] == INF) {
                        if (x.se) {
                            dijk.push_back(mp(curr.fi + x.se, x.fi));
                        } else {
                            dijk.push_front(mp(curr.fi + x.se, x.fi));
                        }
                    }
                }
            }
