...
// to minimize
    ...
    // A lot of typedefs
        ...
        typedef priority_queue<pair<int,pii>, vector<pair<int,pii>>, greater<pair<int,pii>>> pqdijk;
...
int main ()
{
    ...
    dijk.push(mp(0, mp(1, 1)));
    while(dijk.size() > 0) {
        auto curr = dijk.top();
        dijk.pop();
        if (dist[curr.se.fi] == INF || dist[curr.se.fi] == curr.fi) {
            if (curr.se.fi != curr.se.se) {
                EL.pb(mp(curr.se.fi, curr.se.se));
            }
        }
        if (dist[curr.se.fi] == INF) {
            dist[curr.se.fi] = curr.fi;
            for (auto x: AL[curr.se.fi]) {
                if (dist[x.fi] == INF || dist[x.fi] == curr.fi + x.se) {
                    dijk.push(mp(curr.fi+x.se, mp(x.fi, curr.se.fi)));
                }
            }
        }
    }
    ...
    for (auto x: tm) {
        ...
        DinicMaxFlow<int> mf(n+1);
        for (auto edge: EL) {
            mf.addEdge(edge.fi, edge.se, 1);
        }
        for (auto src: loc) {
            // cout << src.fi << " " << src.se << endl;
            mf.addEdge(0, src.fi, src.se);
        }
        int flow = mf.calcMaxFlow(0, 1);
        ttl += flow;
        // cout << flow << endl;
        // cout << "=============" << endl;
    }
