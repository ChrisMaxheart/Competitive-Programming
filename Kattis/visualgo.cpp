...
int main ()
{
    ...
    dijk.push(mp(0, mp(s,s)));

    while(dijk.size() > 0) {
        auto curr = dijk.top();
        dijk.pop();
        if (dist[curr.se.se] < curr.fi) {
            continue;
        } else if (dist[curr.se.se] == curr.fi) {
            num[curr.se.se]+= num[curr.se.fi];
        } else {
            num[curr.se.se] = num[curr.se.fi];
            dist[curr.se.se] = curr.fi;
            for (auto x : AL[curr.se.se]) {
                if (dist[x.fi] >= x.se+curr.fi) {
                    dijk.push(mp(x.se+curr.fi, mp(curr.se.se, x.fi)));
                }
            }
        }
