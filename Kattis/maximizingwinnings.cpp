...
int recurs(int pos, int turn) {
    ...
    if (mindp[turn][pos] == -1) {
        ...
        for (int i = 0; i < N; i++) {
            int curr = recurs(i, turn-1) + AM[pos][i];
            mini = min(mini, curr);
        }
        mindp[turn][pos] = mini;
    }
    return mindp[turn][pos];
}

int mrecurs(int pos, int turn) {
    if (turn == 0) {
        return 0;
    }
    ...
    if (maxdp[turn][pos] == -1) {
        ...
        for (int i = 0; i < N; i++) {
            int curr = mrecurs(i, turn-1) + AM[pos][i];
            maxi = max(maxi, curr);
        }
        maxdp[turn][pos] = maxi;
    }
    return maxdp[turn][pos];
}
...
int main ()
{
    ...
    while(cin >> N, N) {
        ...
        cout << mrecurs(0, m) << " " << recurs(0,m) << endl;
