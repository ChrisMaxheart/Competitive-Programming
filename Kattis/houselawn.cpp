...
bool passed(int A, int Ax, int t, int tR) {
    return ll(A) * ll(t+tR) <= ll(10080) * ll(t) * ll(Ax);
}
...
int main ()
{
    ...
    for (int i = 0; i < N; i++) {
        getline(cin, dummy);
        vs splitted = strsplit(dummy, ',');
        string name = splitted[0];
        int price = stoi(splitted[1]);
        int Ax = stoi(splitted[2]);
        int t = stoi(splitted[3]);
        int tR = stoi(splitted[4]);
        if (passed(A, Ax, t, tR)) {
            passed_lst.eb(name, price);
            mini = min(mini, price);
        }
    }
    if (passed_lst.empty()) {
