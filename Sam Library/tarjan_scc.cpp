...
// update: bug cleared, tarjanCheck no longer needed
#define UNVISITED  -1 // warning, the bug will triggers if unvisited == 0, use -1 instead
vector<vi> final;
vector<vector<pii>> AL; // fi is node, se is distance
...
// if unvisited == -1, this check is unnecessary
void tarjanCheck() {
    int bug = 0;
