...
impl Solver {
    fn solve(&mut self) -> i32 {
        self.data[1] = self.noun;
        self.data[2] = self.verb;
...
fn main() {
    let inp = &InputReader{ filename:String::from("./test.txt") }.read()[0];
    for verb in 0..100 {
        for noun in 0..100 {
            let mut solver = Solver{ data:inp.split(",").map(|x| x.parse::<i32>().unwrap()).collect(), verb, noun };
