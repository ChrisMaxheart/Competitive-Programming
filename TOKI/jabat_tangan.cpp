...
umapllll jabat_tangan_memo;
ll jabat_tangan(ll n) {
    if (n == 0) {
        return 1;
    }
    if (n % 2) {
    ...
    if (jabat_tangan_memo.find(n) == jabat_tangan_memo.end()) {
        ll sum = 0;
        for (int i = 0; i < n-1; i++) {
            sum += (jabat_tangan(i) * jabat_tangan(n-2-i));
        }
        jabat_tangan_memo[n] = sum;
    }
    return jabat_tangan_memo[n];
}
