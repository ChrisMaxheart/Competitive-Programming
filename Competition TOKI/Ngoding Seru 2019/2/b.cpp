...
int main ()
{
    ...
    vi tim;
    vector<vector<pair<int, pii>>> memo;
    ...
    DinicMaxFlow<int> mf(N + T + K + 2);
    int src = N + K + T;
    int snk = N + K + T + 1;

    for (int i = 0; i < N; i++) {
        // cout << "sambung " << src << " dengan " << i << " kapasitas " << 1 << endl;
        mf.addEdge(src, i, 1);
    }
    ...
    for (int i = 0; i < K; i++) {
        ...
        for (int j = 0; j < tim[i]; j++ ) {
            int timloc = N + curr + j;
            // cout << "sambung " << timloc << " dengan " << lomba << " kapasitas " << 1 << endl;
            mf.addEdge(timloc, lomba, 1);

            pair<int, pii> org = memo[i][j];
            // cout << "sambung " << org.fi << " dengan " << timloc << " kapasitas " << 1 << endl;
            // cout << "sambung " << org.se.fi << " dengan " << timloc << " kapasitas " << 1 << endl;
            // cout << "sambung " << org.se.se << " dengan " << timloc << " kapasitas " << 1 << endl;
            mf.addEdge(org.fi, timloc, 1);
            mf.addEdge(org.se.fi, timloc, 1);
            mf.addEdge(org.se.se, timloc, 1);
        }
        curr += tim[i];
        // cout << "sambung " << lomba << " dengan " << snk << " kapasitas " << tim[i] << endl;
        mf.addEdge(lomba, snk, tim[i]);
    }
